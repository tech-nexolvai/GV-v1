...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("rule_id <> ''", name="ck_rule_definitions_rule_definition_rule_id"),
        sa.PrimaryKeyConstraint("id", name="pk_rule_definitions"),
        sa.UniqueConstraint("rule_id", name="uq_rule_definitions_rule_id"),
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "snapshot_id LIKE 'sha256:%'", name="ck_rule_snapshots_rule_snapshot_id_is_prefixed"
        ),
        sa.CheckConstraint(
            "canonical_json <> ''", name="ck_rule_snapshots_rule_snapshot_canonical_json"
        ),
        sa.CheckConstraint("version <> ''", name="ck_rule_snapshots_rule_snapshot_version"),
        sa.CheckConstraint(
            "product_type <> ''", name="ck_rule_snapshots_rule_snapshot_product_type"
        ),
        sa.CheckConstraint(
            "unconfirmed_tolerance_count >= 0",
            name="ck_rule_snapshots_rule_snapshot_unconfirmed_tolerance_count",
        ),
        ...
        sa.UniqueConstraint("snapshot_id", name="uq_rule_snapshots_snapshot_id"),
        sa.UniqueConstraint(
    ...
    op.create_index("ix_rule_snapshots_snapshot_id", "rule_snapshots", ["snapshot_id"])
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "discriminator <> ''",
            name="ck_rule_applicability_scopes_rule_applicability_scope_discriminator",
        ),
        sa.CheckConstraint(
            f"discriminator NOT IN ({_RESERVED})",
            name="ck_rule_applicability_scopes_rule_applicability_scope_discriminator_not_reserved",
        ),
        sa.CheckConstraint(
            "value <> ''", name="ck_rule_applicability_scopes_rule_applicability_scope_value"
        ),
        sa.ForeignKeyConstraint(
            ["rule_snapshot_id"],
            ["rule_snapshots.id"],
