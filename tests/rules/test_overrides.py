...
def test_a_project_override_of_a_company_standard_is_reported() -> None:
    ...
    assert "overrides 4 in (global)" in override.explain()
...
def test_the_override_module_cannot_reach_extraction_or_evidence() -> None:
    ...
    """
    source = (Path(__file__).resolve().parents[2] / "rules" / "overrides.py").read_text()

    for forbidden in ("extraction", "evidence", "retrieval", "app."):
...
def test_a_displaced_fraction_is_also_written_as_a_mixed_number() -> None:
    ...
    assert "overrides 1 1/2 in (global)" in line
