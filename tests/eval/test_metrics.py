...
def test_the_note_explains_v1_flag_policy_and_future_evidence_need() -> None:
    """The intentionally unmeasured V1 metric must name its actual prerequisites."""
    note = critical_false_pass_rate([], []).note

    assert "FLAG" in note
    assert "Q4" in note
    assert "#274" in note
    assert "rulebook does declare CRITICAL" not in note
