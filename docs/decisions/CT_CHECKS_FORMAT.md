...
- **`CT004` is the sink-CABINET interior width** (cutout + two clearances + two side panels) — Q19.
...
1. **`B.S_THK` (backsplash thickness) is a NEW variable.** It sits inside the countertop depth and must
   be subtracted in the back-offset remainder. Our current `ct_back_offset_min` subtracts only front
   offset + sink depth — it is **missing overhang (`C.T_OH`) and backsplash (`B.S_THK`)**.
